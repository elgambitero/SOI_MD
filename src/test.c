...
void TST_game_config(game_config_t config){
    ...
    sprintf(text_buffer, "%05d", config.start_level);
    VDP_drawText(text_buffer, GMCFG_INDENT, 1);

    sprintf(text_buffer, "%05d", config.start_level_set);
    VDP_drawText(text_buffer, GMCFG_INDENT, 2);

    sprintf(text_buffer, "%05d", config.num_players);
...
void TST_print_game_status_header(){
    ...
    VDP_drawText("Player 1: ", 0, 3);
    VDP_drawText("score: " , 0, 4);
    VDP_drawText("bonus: " , 0, 5);
    VDP_drawText("mult: " , 0, 6);
    VDP_drawText("arrows: " , 0, 7);
    VDP_drawText("balls: " , 0, 8);
    VDP_drawText("lives: " , 0, 9);
}
...
{
    sprintf(text_buffer, "%05d" , status->player_stat[player_index].score);
    VDP_drawText(text_buffer, GMSTS_INDENT, 4 + player_index * 6);
    sprintf(text_buffer, "%05d" , status->player_stat[player_index].bonus);
    VDP_drawText(text_buffer, GMSTS_INDENT, 5 + player_index * 6);
    sprintf(text_buffer, "%05d" , status->player_stat[player_index].mult);
    VDP_drawText(text_buffer, GMSTS_INDENT, 6 + player_index * 6);
    sprintf(text_buffer, "%05d" , status->player_stat[player_index].arrows);
    VDP_drawText(text_buffer, GMSTS_INDENT, 7 + player_index * 6);
    sprintf(text_buffer, "%05d" , status->player_stat[player_index].balls);
    VDP_drawText(text_buffer, GMSTS_INDENT, 8 + player_index * 6);
    sprintf(text_buffer, "%05d" , status->player_stat[player_index].lives);
